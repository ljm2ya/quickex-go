...
type BybitFuturesClient struct {
	*core.WsClient
	client    *bybit.Client
	apiKey    string
	apiSecret string
...
func NewClient(apiKey, apiSecret string) *BybitFuturesClient {
...
func nextWSID() string {
	return fmt.Sprintf("%d-%d", time.Now().UnixMilli(), rand.Intn(100))
}

// --- 인증 메시지 작성 ---
func (c *BybitFuturesClient) authFn() core.WsAuthFunc {
...
// --- 유저 데이터 핸들러 ---
func (c *BybitFuturesClient) userDataHandlerFn() core.WsUserEventHandler {
...
func (c *BybitFuturesClient) afterConnect() core.WsAfterConnectFunc {
...
// Bybit Futures format: BTCUSDT (no separator)
func (c *BybitFuturesClient) ToSymbol(asset, quote string) string {
	return asset + quote
}

// ToAsset extracts the asset from a symbol (reverse of ToSymbol)
func (c *BybitFuturesClient) ToAsset(symbol string) string {
	// Bybit Futures uses simple concatenation: BTCUSDT
